...
class BFT_Event_Tickets_List extends WP_List_Table {
	...
	public static function get_event_query( $event_id = 0, $event_tickets_only = false ) {
		...
		$sql .= "IN (SELECT 1 FROM {$wpdb->prefix}bft_ticket t WHERE t.FK_EventID = {$event_id})";
		return $sql;
	}
	 ...
	 * @return mixed
	 */
	public static function get_tickets( $event_id, $event_tickets_only, $per_page = 5, $page_number = 1 ) {

		global $wpdb;

		$sql = self::get_event_query($event_id, $event_tickets_only);
	 ...
	 * @return null|string
	 */
	public static function record_count($event_id, $event_tickets_only) {
		global $wpdb;
	...
	function column_post_title( $item ) {
		error_log("yy".print_r($item, true));
	 ...
	 *
	 */
	public function prepare_items($event_id, $event_tickets_only) {
		...
		$total_items  = self::record_count($event_id, $event_tickets_only);
		...
		$this->items = self::get_tickets( $event_id, $event_tickets_only, $per_page, $current_page );
	}
